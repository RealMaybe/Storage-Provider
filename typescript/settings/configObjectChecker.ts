...
// 导入依赖
import { type TypeUserConfigObject } from "../type/typeClassConfig";
 ...
 * 
 * @param { TypeUserConfigObject } settings 要验证的设置对象。
 * 
 * @returns { { valid: boolean, value: TypeUserConfigObject } } 如果所有必需属性都存在，则返回验证后的设置对象。
 * 
 * @throws { Error } 如果设置对象无效或缺少任何必需属性，则抛出错误。
 */
export function configObjectChecker(settings: TypeUserConfigObject): {
    valid: true,
    value: TypeUserConfigObject
} {
    const CONFIG = ((set: any): TypeUserConfigObject => {
    ...
    // 必需属性
    const requiredAttributes: Array<string> = ["storageType", "warn"];
