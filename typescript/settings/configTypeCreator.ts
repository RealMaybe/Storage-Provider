...
type ValidatorWithErrorMessage = {
 ...
 * @returns { ValidatorWithErrorMessage } 包含布尔值验证器和错误信息的对象
 */
const booleanValidator = (name: string): ValidatorWithErrorMessage => ({
    validator: (value: any): boolean => typeof value === "boolean",
 ...
 * @param { string } name 属性名称
 * @returns { ValidatorWithErrorMessage } 包含字符串验证器和错误信息的对象
 */
const stringValidator = (minLength: number, maxLength: number, name: string): ValidatorWithErrorMessage => ({
    validator: (value: any): boolean =>
        typeof value === "string" &&
 ...
 * @returns { ValidatorWithErrorMessage } 包含数字验证器和错误信息的对象
 */
const numberValidator = (conditions: Array<string>, errorMessage: string): ValidatorWithErrorMessage => ({
    validator: (value: any): boolean => {
        /**
         * 尝试将条件字符串转换为对象
         ...
         */
        const splitByOperator = (str: string): {
            operator: string,
            number: number
        } | null => {
            const regex = /(<=|>=|<|>|==|!=)(\d+)/;
 ...
 * @param { ValidatorWithErrorMessage } validatorObject 包含验证器和错误信息的对象
...
>(
    type: K, // 确保 type 是一个字符串字面量类型
    required: boolean,
    validatorObject: ValidatorWithErrorMessage,
    defaultValue: TypeFromMapping<K> // 确保 defaultValue 的类型与 type 一致
): ConfigItem<K> {
    return {
        type, // 类型
        required, // 是否必填
...
export const typeCreator: TypeClassConfigDefaultValidator = {
    // 存储类型
