...
type inspectorAttribute = keyof inspectorObject;
 ...
 * 
 * @param { TypeClassConfig } classConfig - 配置对象，包含存储信息和警告标志
 * @param { inspectorObject } obj - 包含需要验证的键值对的对象
 * 
...
export function m_inspector(
    ...
    const validatedObj: inspectorObject = ValidateObject(classConfig, obj);
    const keys: Array<string> = Object.keys(validatedObj);
    ...
    // 定义有效的类型集合
    const validTypes: Set<string> = new Set(["string", "number", "boolean", "function", "array", "object"]);
    // 存储提示信息的对象
    let tips: { [storageKey: string]: string } = {};
    // 存储错误信息的对象
    let errors: { [storageKey: string]: string } = {};

    // 从存储中获取多个值
    const storageData: {
        [key: string]: any;
    } = m_getMany(classConfig, keys, "object");

    // 遍历每个键进行验证
    keys.forEach((storageKey: string) => {
        const validator: inspectorObject[inspectorAttribute] = validatedObj[storageKey]; // 获取验证规则
        const value: any = storageData[storageKey]; // 获取存储中的值
