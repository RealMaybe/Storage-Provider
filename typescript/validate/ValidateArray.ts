 ...
 */
export function ValidateArray<T extends Array<any>>(
    ...
    // 允许的类型列表
    const allowedTypes: Array<string> = ["array", "bigint", "boolean", "function", "null", "number", "object", "string", "symbol", "undefined"];
    ...
    if (type !== null) {
        ...
        const isValidType = type === "array" ?
            (item: T) => Array.isArray(item) :
            (item: any) => typeof item === type;

        if (arr.some((item: T) => !isValidType(item)))
